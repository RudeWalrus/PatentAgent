...
describe PatentAgent::USUrls do
  let(:num) {"6266379"}
  let(:pnum) {PatentAgent::PatentNum.new(num)}

  context "Private Methods" do 
    it "Returns the number part of a patent" do
      number = PatentAgent::USUrls.send(:get_num_part_of_patent, pnum)
      number.should == num
    end
    it "Accepts a string & returns the number part of a patent" do
      number = PatentAgent::USUrls.send(:get_num_part_of_patent, "US#{pnum}")
      number.should == num
    end
  end
  ...
  it "Generates a US url" do
    ...
    PatentAgent::USUrls.patent_url(pnum).should eq path
  end

  it "generates a forward citation url" do
    url = PatentAgent::USUrls.fc_url(pnum, 3)
